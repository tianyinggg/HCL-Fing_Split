#!/usr/bin/env python3
"""Run MKL baseline only on shared tiny CSR sanity matrices."""
...
BENCH_DIR = REPO_ROOT / "benches" / "mkl"
BIN = BENCH_DIR / "mkl_baseline"
...
def read_latest_row(output: Path, matrix_name: str, mode: str) -> dict[str, str]:
    ...
    with output.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            if (
                row.get("mode") == mode
                and row.get("method") == "mkl"
                and row.get("matrix") == matrix_name
            ):
                rows.append(row)
    if not rows:
        raise RuntimeError(f"no mkl CSV row found for {matrix_name}")
    return rows[-1]
...
def main() -> int:
    ...
    if failures:
        print("MKL tiny sanity failed:", file=sys.stderr)
        for failure in failures:
            print(f"  {failure}", file=sys.stderr)
        return 1

    print(f"MKL {args.mode} tiny sanity passed for {len(matrices)} matrix/matrices")
