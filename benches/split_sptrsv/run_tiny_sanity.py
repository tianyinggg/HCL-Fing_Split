#!/usr/bin/env python3
"""Run Split_SpTRSV baseline only on shared tiny CSR sanity matrices."""
...
BENCH_DIR = REPO_ROOT / "benches" / "split_sptrsv"
BIN = BENCH_DIR / "split_sptrsv_baseline"
...
def read_latest_row(output: Path, matrix_name: str, mode: str) -> dict[str, str]:
    ...
    with output.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            if (
                row.get("mode") == mode
                and row.get("method") == "split_sptrsv"
                and row.get("matrix") == matrix_name
            ):
                rows.append(row)
    if not rows:
        raise RuntimeError(f"no split_sptrsv CSV row found for {matrix_name}")
    return rows[-1]
...
def main() -> int:
    ...
    parser.add_argument("--fresh", action="store_true", help="运行前删除输出 CSV")
    ...
    if failures:
        print("Split_SpTRSV tiny sanity failed:", file=sys.stderr)
        for failure in failures:
            print(f"  {failure}", file=sys.stderr)
        return 1

    print(f"Split_SpTRSV {args.mode} tiny sanity passed for {len(matrices)} matrix/matrices")
