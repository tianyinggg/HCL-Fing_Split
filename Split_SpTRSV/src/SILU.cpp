...
static void fill_csr_diag(const csr_matrix *mat, val_type *diag)
{
...
static void fill_csc_diag(const csc_matrix *mat, val_type *diag)
{
    for(int col = 0; col < mat->n; col++)
    {
        diag[col] = 1.0;
        for(int j = mat->cscColPtr[col]; j < mat->cscColPtr[col+1]; j++)
        {
            if(mat->cscRowIdx[j] == col)
            {
                diag[col] = mat->cscVal[j];
                break;
            }
        }
    }
...
SILU::SILU()
{
    ...
    CUSPARSE_RT_CALL(cusparseSetMatDiagType(descrL, CUSPARSE_DIAG_TYPE_NON_UNIT));
    ...
    CUSPARSE_RT_CALL(cusparseSetMatDiagType(descrL1, CUSPARSE_DIAG_TYPE_NON_UNIT));
...
{
  std::string file = "policy/" + filename + ".txt";
...
int SILU::algorithm_analysis()
{
    ...
    if(policy.sptrsv_method2 == CUSPARSE_V2 && device_csr_matrix_bottom.m > 0)
    {
  ...
  if(direction == SUBSTITUTION_FORWARD)
	{
		...
		int stat = HTSExecutor(&device_csr_matrix_top);
            ...
            if(policy.spmv_method == SPMV_CSR && policy.spmv_platform == SPMV_GPU)
            {
                int status = 0;                
                start_profile_GPU();
                if(device_csr_matrix_mvp.m > 0 && device_csr_matrix_mvp.nnz > 0)
                {
                    val_type *spmv_y = d_b_1;
                    if(policy.sptrsv_method2 == SLFC || policy.sptrsv_method2 == ELMC)
                        spmv_y = d_x_1;

                    size_t spmvBufferSize = 0;
                    void *spmvBuffer = NULL;
                    CUSPARSE_RT_CALL(cusparseCsrmvEx_bufferSize(cusparseHandle, CUSPARSE_ALG_MERGE_PATH, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            device_csr_matrix_mvp.m, device_csr_matrix_mvp.n,
                                            device_csr_matrix_mvp.nnz, &minusdoubleone, CUDA_R_64F, descrL,
                                            device_csr_matrix_mvp.d_csrVal, CUDA_R_64F, device_csr_matrix_mvp.d_csrRowPtr,
                                            device_csr_matrix_mvp.d_csrColIdx, d_x,
                                            CUDA_R_64F, &doubleone, CUDA_R_64F, spmv_y, CUDA_R_64F,
                                            CUDA_R_64F, &spmvBufferSize));
                    if(spmvBufferSize > 0)
                        CUDA_RT_CALL(cudaMalloc(&spmvBuffer, spmvBufferSize));
                    cusparseStatus = cusparseCsrmvEx(cusparseHandle, CUSPARSE_ALG_MERGE_PATH, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            device_csr_matrix_mvp.m, device_csr_matrix_mvp.n, 
                                            device_csr_matrix_mvp.nnz, &minusdoubleone, CUDA_R_64F, descrL,
                                            device_csr_matrix_mvp.d_csrVal, CUDA_R_64F, device_csr_matrix_mvp.d_csrRowPtr,
                                            device_csr_matrix_mvp.d_csrColIdx, d_x,
                                            CUDA_R_64F, &doubleone, CUDA_R_64F, spmv_y, CUDA_R_64F,
                                            CUDA_R_64F, spmvBuffer);
                    if(spmvBuffer != NULL)
                        CUDA_RT_CALL(cudaFree(spmvBuffer));
                }
  ...
  printf("Submatrix Rows\n");	
    ...
    if(policy.sptrsv_method1 == CUSPARSE_V2 || policy.sptrsv_method1 == ELMR)
	{
      ...
      if(policy.sptrsv_method1 == ELMR)
      {
        ...
        CUDA_RT_CALL(cudaMemset(ready_1, 0, TopTri_csr.m * policy.rhs * sizeof(char)));
        free(diag);
	...
	CUDA_RT_CALL(cudaMalloc((void **)&device_csr_matrix_bottom.d_csrColIdx, BotTri_csr.nnz  * sizeof(ind_type)));
        ...
        if(policy.sptrsv_method2 == ELMR)
	  {
	    ...
	    val_type *diag = (val_type *)malloc(sizeof(val_type) * BotTri_csr.m);
           ...
           CUDA_RT_CALL(cudaMemset(ready_2, 0, BotTri_csr.m * policy.rhs * sizeof(char)));
           free(diag);
...
{	
    return SetRefVars(x_ref, b, inmat, rhs, ref_mode);
}
