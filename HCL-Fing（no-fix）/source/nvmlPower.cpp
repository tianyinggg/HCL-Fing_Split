...
void *powerPollingFunc(void *ptr)
{
	...
	while (pollThreadStatus)
	{
		...
		// fprintf(fp, "%s ----- %.3lf\n",ptr, (powerLevel)/1000.0);
		pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, 0);

		avg_power = avg_power + (1 / (double)iter++) * (powerLevel - avg_power);
	}

	// fclose(fp);
	
	long int_avg_power = round(avg_power);
	pthread_exit((void*) int_avg_power);
}
...
*/
void nvmlAPIRun(const char * kernel_name)
{
	int i;
	...
	int iret = pthread_create(&powerPollThread, NULL, powerPollingFunc, (void*) kernel_name);
	if (iret)
	{
...
float nvmlAPIEnd()
{
	...
	pthread_join(powerPollThread, &power);
